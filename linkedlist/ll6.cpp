//Creation of linkedlist and add a node before given data
#include <stdio.h>
#include <iostream>
#include <cstdlib>
using namespace std;
...
//Adding a node after given data node
...
//Adding a node before given data node
void add_node_before_given_data(int x, int y)
{
    temp=first;
    while(temp->data!=x)
    {
...
int main()
{
   ...
   while (1)
    {
        cout << "Do you want to add a node before given data= ";
        cin >> i;
        if (i == 'n' || i == 'N')
            break;
        else
        ...
        cout << "Enter data for New Node= ";
        cin >> m;
        add_node_before_given_data(n, m);
    }
